...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    ...
    setLoading(true);
    try {
      const result = await registerUser(data.email, data.password, data.business_name);
      if (result.success) {
        navigate('/dashboard');
      } else {
    ...
    <div className="min-h-screen bg-[#fcfcfc] flex flex-col md:flex-row antialiased selection:bg-pink-200 overflow-hidden font-sans">
      ...
      <div className="hidden md:flex md:w-[50%] bg-black relative flex-col justify-between p-24 border-r border-white/10 overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center space-x-4 cursor-pointer group mb-32" onClick={() => navigate('/')}>
          ...
          <div className="space-y-10">
            <div className="inline-flex items-center space-x-3 px-5 py-2.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-md">
               ...
               <span className="text-[11px] font-black uppercase tracking-[0.5em] text-gray-400">System Live</span>
            </div>
            <h1 className="text-[110px] font-black tracking-tighter text-white leading-[0.75] uppercase select-none">
              Start Your <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-gray-100 via-gray-400 to-gray-700 italic">Evolution.</span>
            </h1>
          </div>
        </div>
        ...
        <div className="relative z-10 flex items-center justify-between border-t border-white/10 pt-12">
          <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">
            © 2024 PRICELENS ANALYTICS LABS.
          </p>
          <ShieldCheck className="h-10 w-10 text-white/10" />
        </div>
      </div>

      {/* --- STRETCHED REGISTER CONTAINER --- */}
      <div className="flex-1 flex items-center justify-center p-6 md:p-12 relative">
        {/* Container stretched to max-width 850px */}
        <Card className="w-full max-w-[850px] bg-white border border-gray-100 rounded-[4rem] shadow-[0_50px_120px_rgba(0,0,0,0.08)] overflow-hidden transition-all hover:shadow-[0_60px_140px_rgba(0,0,0,0.1)] group/card">
          <CardContent className="p-12 md:p-20 relative">
            ...
            <div className="mb-16 relative z-10">
              <h2 className="text-5xl font-black tracking-tighter text-black uppercase mb-4">Register.</h2>
              <div className="h-2 w-16 bg-[#D95B96] rounded-full" />
              <p className="mt-6 text-gray-400 text-xl font-medium tracking-tight leading-relaxed italic">Begin your 14-day free trial. No credit card required.</p>
            </div>

            {error && (
              <Alert variant="destructive" className="mb-10 bg-red-50 border-red-100 text-red-600 rounded-3xl relative z-10 py-6">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-10 relative z-10">
              {/* Horizontal grid for stretch effect */}
              <div className="grid md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4 group/input">
                  <Label className="text-[12px] font-black text-gray-400 uppercase tracking-[0.3em] ml-2 transition-colors group-focus-within/input:text-[#D95B96]">Business Name</Label>
                  <Input
                ...
                <div className="space-y-4 group/input md:col-span-2">
                  <Label className="text-[12px] font-black text-gray-400 uppercase tracking-[0.3em] ml-2 transition-colors group-focus-within/input:text-[#D95B96]">Password *</Label>
              ...
              <div className="grid grid-cols-4 gap-4 mt-6 px-1">
                {[
                  ...
                  <div key={i} className="flex flex-col items-center">
                    <div className={`h-1.5 w-full rounded-full transition-colors duration-500 ${req.check ? 'bg-[#D95B96]' : 'bg-gray-100'}`} />
                    <span className="text-[9px] font-black uppercase tracking-tighter text-gray-400 mt-3">{req.label}</span>
                  </div>
                ))}
              </div>
              ...
              <button
                ...
                type="submit"
                disabled={loading}
                className="group relative w-full h-24 bg-black text-white rounded-[3rem] font-black uppercase tracking-[0.25em] text-sm transition-transform duration-200 ease-out flex items-center justify-center overflow-hidden shadow-2xl active:scale-[0.98] mt-12"
              >
            ...
            <div className="mt-20 text-center relative z-10">
              <p className="text-gray-400 text-[12px] font-black uppercase tracking-[0.3em] leading-relaxed">
                ...
                <Link to="/login" className="text-black border-b-2 border-[#D95B96] hover:border-black transition-all pb-1.5 ml-3">
                  Sign In here
                </Link>
              </p>
            </div>
